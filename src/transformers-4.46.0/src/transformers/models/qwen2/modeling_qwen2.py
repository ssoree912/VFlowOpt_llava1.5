...
class Qwen2Model(Qwen2PreTrainedModel):
    ...
    # -------------使用vit的attn----------------
        VIT_ATTN_MAP = torch.tensor(json.loads(os.environ["VIT_ATTN_MAP"]))
        combined_attn = VIT_ATTN_MAP.to(hidden_states.device)
        for layer_idx, decoder_layer in enumerate(self.layers):
            if output_hidden_states:
                all_hidden_states += (hidden_states,)
            ...
            if use_cache:
                if hidden_states.shape[1] != 1:
                    if layer_idx<illava_llm_k[0]:
                        pruned_attention_mask = causal_mask
                    elif layer_idx in illava_llm_k:
                        if not del_musks:
                            ...
                            for r_val in illava_llm_r:
                                ...
                                del_musks.append(del_musk)

                                combined_attn = combined_attn[del_musk[illava_llm_image_token_start_index:illava_llm_image_token_start_index + image_token_length]]

                                image_token_length -= r
                                len_hidden_states -= r

                        del_musk = del_musks[illava_llm_k.index(layer_idx)]
                    
                    # -------------------------------------------
                        hidden_states = hidden_states[:,del_musk.to(hidden_states.device),:]
                        new_seq_length = hidden_states.shape[1]

                        # update position ids
                        # position_ids = keep_indexs.unsqueeze(0)
                        position_ids = position_ids[:, del_musk]
                        position_embeddings = self.rotary_emb(hidden_states, position_ids)
                ...
                if layer_idx<illava_llm_k[0]:
                    ...
                    print(f"hidden_states != 1, with shape:{hidden_states.shape}")
                    ...
                    last_layer_attention_avg_last_tok_image = last_layer_attention_avg_last_tok[illava_llm_image_token_start_index:illava_llm_image_token_start_index+image_token_length]
                    ...
                    values = hidden_states[0, illava_llm_image_token_start_index+indices[image_token_length-(r+1):]]
                    # generate predefined weights
                    size = torch.arange(1, r+2)  # from r+1 to 1
                    # merge tokens
                    hidden_states[0, illava_llm_image_token_start_index+indices[image_token_length-(r+1)]] = (size.double().to(values.get_device()) @ values.double() / size.double().sum().to(values.get_device())).half()
                    set_selected = list(indices[:image_token_length-r])
                    set_selected.sort()
                    # show index
                    top_attention_rank_index = [ k + illava_llm_image_token_start_index for k in set_selected]
                    # keep index
                    keep_indexs = torch.cat( (torch.arange(illava_llm_image_token_start_index,device=device), torch.tensor(top_attention_rank_index, device=device), torch.arange(illava_llm_image_token_start_index+image_token_length,seq_length_with_past,device=device)))
                    # sort index

...
from transformers import CLIPSegProcessor, CLIPSegForImageSegmentation, CLIPModel, CLIPImageProcessor
    ...
    if "llava" in model_name.lower() or is_multimodal:
        ...
        if "lora" in model_name.lower() and model_base is not None:
            ...
            if "mixtral" in model_name.lower():
                ...
                elif 'training_free' in model_name.lower():
                    from llava.model.language_model.llava_qwen_training_free import LlavaQwenTrainingFreeConfig
                    if overwrite_config is not None:
                        llava_cfg = LlavaQwenTrainingFreeConfig.from_pretrained(model_path)
                        rank0_print(f"Overwriting config with {overwrite_config}")
                        for k, v in overwrite_config.items():
                            setattr(llava_cfg, k, v)
                        model = LlavaQwenTrainingFreeForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True, attn_implementation=attn_implementation, config=llava_cfg, **kwargs)
                        # CLIPSeg
                        # model.CLIPSeg_Processor = CLIPSegProcessor.from_pretrained('CIDAS/clipseg-rd64-refined')
                        # model.CLIPSeg_Model = CLIPSegForImageSegmentation.from_pretrained('CIDAS/clipseg-rd64-refined')
                        # model.CLIPSeg_Model.requires_grad_(False)
                        # Transformer CLIP
                        # model.tokenizer = AutoTokenizer.from_pretrained('openai/clip-vit-base-patch16', device_map=device_map)
                        # model.clip_model = CLIPModel.from_pretrained('openai/clip-vit-base-patch16', device_map=device_map)
                        # model.image_processor = CLIPImageProcessor.from_pretrained('openai/clip-vit-base-patch16')
                        # model.clip_model.requires_grad_(False)
                    else:
                        model = LlavaQwenTrainingFreeForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True, attn_implementation=attn_implementation, **kwargs)
                else:
